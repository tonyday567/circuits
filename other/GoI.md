# Geometry of Interaction via Delimited Continuations

**Status:** Working prototype. Executable model of the Int construction using GHC 9.6+ RTS delimited continuations.

**Connection:** Bridges TracedA (syntax) and HypA (semantics) to runnable traced category code via shift/reset primitives.

---

## Overview

Delimited continuations (prompt/control0 from GHC 9.6+) provide a natural way to implement traced monoidal categories and the Int construction operationally.

**Key insight:** The trace operator (which feeds output back as input via a feedback loop) is structurally identical to a delimited continuation that captures the current stack up to a prompt and allows it to be resumed.

**principalCutIO** demonstrates two machines interacting geometrically:
- Producer yields values
- Consumer receives and processes
- Interaction happens via prompt/control0, not explicit loops
- RTS stack carries state between yields
- Matches the nLab GoI picture: particle-style dynamics as controlled interaction

**References:**
- Hasegawa–Kameyama (ICFP 2003): equational axiomatisation of shift/reset matching trace laws
- Hoshino et al. (2014): Memoryful Geometry of Interaction (continuation monad for effects)
- Alexis King / Lexi Lambda: GHC delimited continuation design and implementation
...
-- | Multi-shot: k called twice. Expected: [10,20]  (confirmed OK)
exampleMultiShot :: IO ()
exampleMultiShot = do
  tag <- newPromptTag
