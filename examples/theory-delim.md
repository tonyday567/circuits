...
| `forall r` in `reset :: DCont a a -> DCont r a` | `forall a` in `Knot :: arr (t a b) (t a c)` |
...
is the reduction rule `catch E[throw e] f ⟶ f e` — `Knot` playing `catch`, `run g` playing the evaluation context `E`, `untrace` injecting into the hole, `trace` collapsing the boundary.
...
`Knot` is that delimiter. The tensor `t` and `forall a` are its type-level witness. The cancellation is valid because `Knot` seals the scope — the fixpoint cannot escape or be confused with a fixpoint from another scope.

---

## not an analogy

The `r` parameter in `DCont` and the tensor `t` in `Circuit` are the same type-level mechanism: tracking the result type of the nearest delimiter, universally quantified at the boundary, fixed within the scope. The reduction rule for `catch E[throw e]` and the Mendler case for `run (Compose (Knot f) g)` are the same rule: extract what is inside the delimiter, apply the boundary operation, continue.
