...
This card walks through the structure of each `Either` instance: the bracket
pattern in `trace`, the uniformity of `untrace`, and what changes between
pure and effectful arrows.
...
The loop mechanism is what differs. The pure instance tail-calls. The
Kleisli instance allocates a `PromptTag`, places a `prompt` boundary,
and uses `control0` on `Left` to capture the continuation and re-enter —
constant stack space regardless of iteration count.

```haskell
-- | Pure Either: tail recursion with Right to exit.
--
-- The bracket decomposed:
-- @
-- trace f b = case f (Right b) of
--   Right c -> c           -- exit: unwrap
--   Left a  -> trace f a   -- loop: re-enter with feedback
-- @
...
The Kleisli instance unpacks the same bracket inside a `do` block. The
ceremony (`newPromptTag`) happens once at setup, then `go` alternates
between `prompt` (boundary) and `control0` (re-entry). See
`Circuit.Traced` lines 132–144 for the full implementation.

```haskell
-- | Kleisli IO Either: delimited continuations with constant stack.
--
...
| branch | `Trace (->) Either` | user-facing `Step s r` (while.md) |
|--------|---------------------|-----------------------------------|
...
```haskell
-- | Pure untrace: the instance is @fmap@, so @untrace f@ lifts @f@
...
```haskell
-- | Kleisli untrace: pass Left through, map effect over Right.
-- >>> runKleisli (untrace (Kleisli (\x -> pure (show x)))) (Right (42 :: Int))
-- Right "42"
-- >>> runKleisli (untrace (Kleisli (\x -> pure (show x)))) (Left (7 :: Int))
-- Left 7
-- >>> runKleisli (right (Kleisli (\x -> pure (show x)))) (Right (42 :: Int))
-- Right "42"
```
...
in `Circuit.Traced` avoids that allocation. Either reads as "pass `Left`
through, map the arrow over `Right`."
...
```haskell
-- | Pure Either: enter Right, fmap runs, exit immediately.
-- >>> trace ((untrace (+ 1)) :: Either Int Int -> Either Int Int) (4 :: Int)
-- 5
--
-- >>> trace ((untrace (\x -> x * 2)) :: Either Int Int -> Either Int Int) (4 :: Int)
-- 8
```

```haskell
-- | Pure (,): second runs inside the knot, exits immediately.
-- >>> trace ((untrace (+ (1 :: Int))) :: (Int, Int) -> (Int, Int)) (4 :: Int)
-- 5
```

```haskell
-- | Kleisli Either: enter Right, map effect, exit immediately.
...
surprising — it just threads the tensor through. Closing that thread
with `trace` recovers exactly what went in. If `trace (untrace f) /= f`,
one of the two methods is wrong.

---

## thinking about trace and untrace
